...
const CartItem = ({ id, amount}) => {
  const { addToCart, decreaseUnit, removeFromCart } = useContext();
  ...
  return (
    <article className="flex bg-stone-100 p-1 border rounded-mg relative">
      ...
      <button onClick={() => removeFromCart(id)} className="text-right text-l absolute top-0 right-2 text-slate-950">
      ...
      <div className="flex flex-col justify-around mx-2">
        ...
        <p className="text-green-700 text-sm">{price}</p>
