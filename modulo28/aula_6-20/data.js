...
export const generateTeamsArray = () => driversInRandomOrder.reduce((acc, currentValue) => {
  const { team, points } = currentValue;
  const teamObject = acc.find(t => t.team === team);
  teamObject ? teamObject.points += points : acc.push({
    team, 
    points
  });
  return acc;
...
// 2a iteração -> [{team: 'red bull racing', points: 575}]
// 3a iteração -> [{team: 'red bull racing', points: 575 + 285}]